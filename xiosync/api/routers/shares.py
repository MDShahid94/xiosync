...
def create_share(payload: CreateShareRequest, request: Request) -> dict[str, Any] | JSONResponse:
    ...
    svc = SharingService(session)
    try:
        rec = svc.create_share(ctx, resource_type=payload.resource_type, resource_id=payload.resource_id,
                               target_org_id=payload.target_org_id, permissions=payload.permissions)
    except Exception as exc:
        return JSONResponse(status_code=422, media_type="application/problem+json", content={
            "type": "https://xiosync.dev/problems/share_error", "title": "Share creation failed",
...
def list_shares(request: Request, resource_type: str | None = None) -> list[dict[str, Any]]:
    ...
    return [{"id": str(r.id), "resource_type": r.resource_type, "resource_id": str(r.resource_id),
...
def revoke_share(share_id: uuid.UUID, request: Request) -> dict[str, Any] | JSONResponse:
    ...
    try:
        ...
        return JSONResponse(status_code=422, media_type="application/problem+json", content={
            "type": "https://xiosync.dev/problems/share_error", "title": "Revoke failed",
            "status": 422, "detail": str(exc),
        })
